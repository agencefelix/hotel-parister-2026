...
class BrowserDetection
{
     ...
     */
    public const string VERSION = '3.74.0';

    /**
     * A type for the version() method indicating a string return value.
     */
    public const string VERSION_TYPE_STRING = 'text';

    /**
     * A type for the version() method indicating a float return value.
     */
    public const string VERSION_TYPE_FLOAT = 'float';
    ...
    {
        foreach (self::$browsers as $browserName => $regEx) {
            if (!empty($this->version($browserName))) {
                return strtolower($browserName);
            }
        }
        return '';
    }
    ...
    public function isMobile($userAgent = null, $httpHeaders = null): bool
    {
        ...
        if ('Amazon CloudFront' === $this->getUserAgent()) {
            ...
            if (array_key_exists('HTTP_CLOUDFRONT_IS_MOBILE_VIEWER', $cfHeaders)
                && 'true' === $cfHeaders['HTTP_CLOUDFRONT_IS_MOBILE_VIEWER']
            ) {
                return true;
            }
        }
    ...
    public function isTablet(string $userAgent = null, array $httpHeaders = null): bool
    {
        ...
        if ('Amazon CloudFront' === $this->getUserAgent()) {
            ...
            if (array_key_exists('HTTP_CLOUDFRONT_IS_TABLET_VIEWER', $cfHeaders)
                && 'true' === $cfHeaders['HTTP_CLOUDFRONT_IS_TABLET_VIEWER']
            ) {
                return true;
            }
        }
        ...
        if (true === isset($properties[$propertyName])) {
            ...
            foreach ($properties[$propertyName] as $propertyMatchString) {
                ...
                if (false === empty($match[1])) {
                    return self::VERSION_TYPE_FLOAT == $type ? $this->prepareVersionNo($match[1]) : $match[1];
                }
            }
        }
