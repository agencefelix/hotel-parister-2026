...
- [ ] ⚠️ **Cache API Figma** : au moment du check final, l'API REST servait encore `lastModified=2026-06-19T15:54` (tags utilisateur récents NON visibles). Re-scanner le balisage quand le cache se rafraîchit (ou après une sauvegarde forçant un bump de version) puis figer la structure (zones/cols, newsletter/social-wall taggés).
